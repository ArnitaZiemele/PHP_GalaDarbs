...
<?php
    ...
    FROM comments c JOIN users u on c.user_id=u.id WHERE c.article_id=$id ORDER BY id DESC";
    $result = mysqli_query($link, $sql) or die('cannot get results!');
    $comments = array();
    ...
    foreach ($comments as $c) {
        foreach ($c as $comment) {
            ?> 
            <div class="comment">
                <p class="font-weight-bold"><?php echo $comment['username']; ?></p>
                <p><i><?php echo $comment['created_at']; ?></i></p>
                <p><?php echo nl2br($comment['content']); ?></p>
                <?php if (isset($_SESSION['username']) && $_SESSION['role'] == 'admin') { ?>
                    <a class="btn btn-danger" href="widgets/comment-delete.php?id=<?php echo $comment['id']; ?>&article-id=<?php echo $id; ?>" onclick="return  confirm('Vēlaties dzēst šo komentāru?')">Dzēst</a>
                <?php } ?>
            </div>    
        <?php }    
    }
